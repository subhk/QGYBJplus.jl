...
QGYBJplus uses a single production stepper: second-order exponential
Runge–Kutta (ETD-RK2). For a semilinear equation
...
Here `L` is diagonal horizontal hyperdiffusion. For a horizontal mode,
...
and the implementation evaluates `e^{-\lambda h}`, `h\varphi_1(-\lambda h)`,
and `h\varphi_2(-\lambda h)` with cancellation-safe series near zero.
Flow and wave fields use their respective hyperdiffusion coefficients.

Advection, refraction, dispersion, and vertical PV diffusion are evaluated
explicitly at both stages. Horizontal hyperdiffusion therefore imposes no
explicit stability restriction, but the advective CFL condition and the
explicit wave/vertical-diffusion limits still constrain `dt`.

```julia
Snp1 = copy_state(Sn)
rk_workspace = ExpRK2Workspace(Sn, plans; G=grid)
exp_rk2_step!(Snp1, Sn, grid, params, plans;
              a=a_vec,
              dealias_mask=mask,
              timestep_workspace=rk_workspace)
```

The high-level `run!` and `run_simulation!` drivers allocate the workspace
and rotate the two state buffers automatically.
